...
function CustomerHomePage() {
  ...
  const fetchNearbyWorkers = async () => {
    try {
      const response = await axios.get("/api/v1/workers/nearby", {
  ...
  const fetchMyJobs = async () => {
    try {
      const response = await axios.get("/api/v1/jobs/my-posts", {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  const fetchCurrentBookings = async () => {
    try {
      const response = await axios.get("/api/v1/bookings/customer/current", {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  const handleJobPost = async (jobData) => {
    try {
      await axios.post("/api/v1/jobs/create", jobData, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
